...
pub struct Strings {
    ...
    pub tab_skills_desc: &'static str,
    pub tab_advanced_desc: &'static str,
    pub tab_about_desc: &'static str,
    pub skills_search: &'static str,
    pub skills_refresh: &'static str,
    pub skills_count: &'static str,
    pub skills_empty: &'static str,
    pub skills_enabled: &'static str,
    pub skills_disabled_label: &'static str,
    pub skills_open_file: &'static str,
    pub skills_open_folder: &'static str,
    pub skills_source: &'static str,
    pub skills_invocable: &'static str,
    pub skills_readonly: &'static str,
    pub skills_toggle_ok: &'static str,
    pub skills_toggle_fail: &'static str,
    pub skills_filter_all: &'static str,
    pub skills_hint_body: &'static str,
    // Tray + notifications
    pub tray_show: &'static str,
    pub tray_hide: &'static str,
    pub tray_quit: &'static str,
    pub tray_section: &'static str,
    pub tray_enable: &'static str,
    pub tray_enable_hint: &'static str,
    pub tray_close_to_tray: &'static str,
    pub tray_close_to_tray_hint: &'static str,
    pub notify_section: &'static str,
    pub notify_on_complete: &'static str,
    pub notify_on_complete_hint: &'static str,
    pub notify_only_unfocused: &'static str,
    pub notify_only_unfocused_hint: &'static str,
    pub notify_turn_title: &'static str,
    pub notify_turn_body: &'static str,
    ...
    pub slash_usage_title: &'static str,
    pub slash_usage_desc: &'static str,
    ...
    pub deny: &'static str,
    pub user_question_title: &'static str,
    pub user_question_submit: &'static str,
    pub user_question_cancel: &'static str,
    pub user_question_chat: &'static str,
    pub user_question_skip: &'static str,
    pub user_question_other: &'static str,
    pub usage_chip: &'static str,
    pub usage_refresh: &'static str,
    pub usage_unknown: &'static str,
    pub announcement_dismiss: &'static str,
    ...
    pub share_modal_title: &'static str,
    pub share_modal_subtitle: &'static str,
    pub share_description: &'static str,
    pub share_homepage: &'static str,
    pub share_download_page: &'static str,
    pub share_copy: &'static str,
    pub share_copied: &'static str,
    pub share_open_download: &'static str,
...
s! {
    ...
    tab_skills_desc: "Browse, enable/disable, and open Grok skills",
        "浏览、启用/禁用、打开 Grok Skills",
    tab_advanced_desc: "Extra agent args and advanced options", "额外 agent 参数与高级选项",
    tab_about_desc: "Version and links", "版本与链接",
    skills_search: "Search skills…", "搜索 Skills…",
    skills_refresh: "Refresh", "刷新",
    skills_count: "skills", "个",
    skills_empty: "No skills found. Install the Grok CLI or add SKILL.md under ~/.grok/skills.",
        "未发现 Skills。请安装 Grok CLI，或在 ~/.grok/skills 下添加 SKILL.md。",
    skills_enabled: "Enabled", "已启用",
    skills_disabled_label: "Disabled", "已禁用",
    skills_open_file: "Open file", "打开文件",
    skills_open_folder: "Open folder", "打开文件夹",
    skills_source: "Source", "来源",
    skills_invocable: "Slash", "斜杠命令",
    skills_readonly: "Bundled (read-only path)", "内置（只读路径）",
    skills_toggle_ok: "Skill preference saved to ~/.grok/config.toml",
        "已写入 ~/.grok/config.toml",
    skills_toggle_fail: "Could not update skill setting", "无法更新 Skill 设置",
    skills_filter_all: "All", "全部",
    skills_hint_body: "Skills are SKILL.md packages discovered by the Grok CLI. Disable writes [skills].disabled in config.toml (agent reloads on next connect).",
        "Skills 是 Grok CLI 扫描的 SKILL.md 包。禁用会写入 config.toml 的 [skills].disabled（下次连接 agent 后生效）。",
    tray_show: "Show Grok Desktop", "显示 Grok Desktop",
    tray_hide: "Hide to tray", "隐藏到托盘",
    tray_quit: "Quit", "退出",
    tray_section: "System tray", "系统托盘",
    tray_enable: "Show tray icon", "显示托盘图标",
    tray_enable_hint: "Keep Grok in the notification area", "在通知区域保留 Grok 图标",
    tray_close_to_tray: "Close button hides to tray", "点关闭时最小化到托盘",
    tray_close_to_tray_hint: "Quit from the tray menu instead of the window X",
        "真正退出请用托盘菜单「退出」",
    notify_section: "Notifications", "消息通知",
    notify_on_complete: "Notify when a turn finishes", "任务/回合完成时通知",
    notify_on_complete_hint: "System toast when the agent finishes a reply",
        "Agent 完成一轮回复后弹出系统通知",
    notify_only_unfocused: "Only when window is in background", "仅窗口不在前台时通知",
    notify_only_unfocused_hint: "Skip toasts while you are actively using the app",
        "正在使用窗口时不打扰",
    notify_turn_title: "Grok finished", "Grok 已完成",
    notify_turn_body: "Agent turn completed — open the app to review.",
        "本轮任务已完成，打开应用查看结果。",
    ...
    slash_usage_title: "Usage", "用量",
    slash_usage_desc: "Ask agent for token usage / cost", "向 Agent 查询 token 用量 / 费用",
    ...
    deny: "Deny", "拒绝",
    user_question_title: "Agent is asking", "Agent 需要你回答",
    user_question_submit: "Submit answers", "提交回答",
    user_question_cancel: "Dismiss", "跳过",
    user_question_chat: "Chat about this", "继续讨论",
    user_question_skip: "Skip interview", "跳过访谈",
    user_question_other: "Other…", "其他…",
    usage_chip: "Usage", "用量",
    usage_refresh: "Refresh /usage", "刷新用量",
    usage_unknown: "No usage data yet", "暂无用量数据",
    announcement_dismiss: "Dismiss", "关闭",
    ...
    share: "Share", "分享",
    share_modal_title: "Share Grok Desktop", "分享 Grok Desktop",
    share_modal_subtitle: "Recommend this visual Grok CLI client", "把这款 Grok CLI 可视化客户端推荐给朋友",
    share_description: "A native visual client for the official Grok CLI. Manage projects, chat sessions, and tool runs without terminal operations.",
        "官方 Grok CLI 的原生可视化客户端，无需终端操作即可管理项目、聊天会话和工具调用。",
    share_homepage: "Project homepage", "项目主页",
    share_download_page: "Latest download", "最新下载",
    share_copy: "Copy share text", "复制分享文案",
    share_copied: "Copied", "已复制",
    share_open_download: "Open download page", "打开下载页",
    ...
    logged_in: "Credentials detected (~/.grok/auth.json)", "已检测到登录凭据 (~/.grok/auth.json)",
